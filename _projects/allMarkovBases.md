...
One observation about the minimal Markov bases we have found is that the set of $$A$$-degrees is the same for both since $$\deg_A (x^2-y)=2$$, $$\deg_A(x^3-z)=3$$ and $$\deg_A (x^2-y)=2$$, $$\deg_A(xy-z)=3$$. Indeed, in general the $$A$$-degrees of a minimal Markov basis are independent of the choice of the minimal Markov basis, see. Thus, we can look at the fibers corresponding to the $$A$$-degrees of any set of minimal generators of $$I_A$$. 
