...
Macaulay2 already contains a package, \textit{FourTiTwo}, which can compute a minimal Markov basis for a given configuration matrix however minimal Markov bases are almost never unique. The method \textit{markovBases} from our package, \textit{allMarkovBases}, builds on top of \textit{toricMarkov} from \textit{FourTiTwo}, incorporating Theorems 2.6 and 2.7 from \cite{CKT_2007} to compute all minimal Markov bases for a given configuration matrix. The package also includes the method \textit{randomMarkov} where, given the \texttt{NumberOfBases} option, the method returns the given number of random minimal Markov bases. Both of these methods start by computing the fibers corresponding to the $A$-degrees of the minimal generators of the toric ideal $I_A$ and this step of the computation can be done by itself using the \textit{fiberGraph} method. The fibers are represented as graphs using the \textit{Graphs} package in order to provide a visualisation of the fibers. \newline

We now present an example. Suppose our configuration matrix is the monomial curve $A=(1 \quad 2 \quad 3)$. The first thing we might want to do is to compute the integer kernel of $A$.
\begin{lstlisting}
...
\end{lstlisting}
Therefore, if we let $x,y,z$ be variables, we know $x^2 -y, x^3 - z \in I_A$. We might suspect that $I_A:= \langle x^u-x^v: u,v \in \NN^3, u-v \in \operatorname{ker}_\ZZ(A)\rangle = \langle x^2 - y, x^3 - z \rangle$ and while the generators of the integer kernel lattice won't in general be a Markov basis (see $A=(3 \quad 4 \quad 5)$), we can use the method \textit{toricMarkov} from the \textit{FourTiTwo} package to confirm that $\{\{2,-1,0\},\{3,0,-1\}\}$ is a Markov basis for $A$. Markov bases are outputted as matrices both in the \textit{FourTiTwo} package and in our package.
...
o4 : Matrix ZZ  <-- ZZ
\end{lstlisting}
We can notice that $xy-z \in I_A$ and $x^3-z \in \langle x^2-y,xy-z \rangle$ so $I_A = \langle x^2-y,xy-z \rangle$ and thus $\{\{2,-1,0\},\{1,1,-1\}\}$ is another Markov basis for $A$. If we run the \textit{markovBases} method from our package, we can confirm that these are in fact the only two minimal Markov bases for $A$.
...
Thus, the \textit{fiberGraph} method in our package starts by computing the $A$-degrees of the one minimal Markov basis computed by \textit{toricMarkov}. Using a breadth-first-search algorithm, it constructs the fibers corresponding to $A$-degrees as graphs where two elements $u,v \in \NN^n$ of a fiber share an edge if they have non-trivial intersection ie. $u_i>0$ and $v_i>0$ for some $i$.
\begin{lstlisting}
i6 : fiberGraph A
...
Applying the algorithm to our example, the connected components of our fibers can be calculated by changing the option \texttt{ReturnConnectedComponents} of \textit{fiberGraph} to \texttt{true}.
\begin{lstlisting}
...
Our package contains the \textit{pruferSequence} method which takes in a Prüfer sequence and outputs the edge set of the corresponding spanning tree using the algorithm from Prüfer's paper. For our example, for either fiber there are two connected components, so to calculate the spanning trees, we input the only Prüfer sequence for $n=2$: the empty sequence. 
\begin{lstlisting}
i8 : pruferSequence {}
...
Finally, our package contains the \textit{randomMarkov} method which firstly calls the \textit{fiberGraph} method in order to construct the connected components of each fibers. Then, for each fiber with $n$ connected components, it generates $n-2$ elements from $\{1,...,n\}$ and uses the \textit{pruferSequence} method to compute the corresponding spanning tree and then outputs the Markov basis computed from this spanning tree and randomly chosen elements from each connected component. This way, Markov bases can be randomly and uniformly selected.
\begin{lstlisting}
...
\end{pmatrix}$, then it is almost 10 times quicker to use \textit{randomMarkov} than to compute all minimal Markov bases and choose one at random.

In addition, if we wanted 3 minimal Markov bases of $B$, we can use the \texttt{NumberOfBases} option.
\begin{lstlisting}
